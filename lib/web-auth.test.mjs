...
test("accepts only the fixed grok username and configured password", async () => {
  ...
  assert.equal(await isValidBasicAuthorization(authorization("grok", "secret"), "secret"), true);
  assert.equal(await isValidBasicAuthorization(authorization("pi", "secret"), "secret"), false);
  assert.equal(await isValidBasicAuthorization(authorization("admin", "secret"), "secret"), false);
  assert.equal(await isValidBasicAuthorization(authorization("grok", "wrong"), "secret"), false);
});
...
test("rejects missing, malformed, and non-canonical authorization values", async () => {
  ...
  assert.equal(await isValidBasicAuthorization(null, "secret"), false);
  assert.equal(await isValidBasicAuthorization("Bearer token", "secret"), false);
  assert.equal(await isValidBasicAuthorization("Basic !!!", "secret"), false);
  assert.equal(await isValidBasicAuthorization(`${valid}!`, "secret"), false);
...
test("does not authenticate when password protection is disabled", async () => {
  ...
  assert.equal(await isValidBasicAuthorization(authorization("grok", "secret"), undefined), false);
});
