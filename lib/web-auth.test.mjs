...
test("accepts only the fixed grok username and configured password", async () => {
  ...
  assert.equal(isValidBasicAuthorization(authorization("grok", "secret"), "secret"), true);
  assert.equal(isValidBasicAuthorization(authorization("pi", "secret"), "secret"), false);
  assert.equal(isValidBasicAuthorization(authorization("admin", "secret"), "secret"), false);
  assert.equal(isValidBasicAuthorization(authorization("grok", "wrong"), "secret"), false);
});

test("supports UTF-8 passwords and colons in the password", async () => {
  const { isValidBasicAuthorization } = await loadSubject();
  const password = "口令:with:colons";
  assert.equal(isValidBasicAuthorization(authorization("grok", password), password), true);
});

test("rejects missing, malformed, and non-canonical authorization values", async () => {
  const { isValidBasicAuthorization } = await loadSubject();
  const valid = authorization("grok", "secret");
...
test("does not authenticate when password protection is disabled", async () => {
  const { isValidBasicAuthorization } = await loadSubject();
  assert.equal(isValidBasicAuthorization(authorization("grok", ""), ""), false);
  assert.equal(isValidBasicAuthorization(authorization("grok", "secret"), undefined), false);
});
...
test("file password authenticates when env is unset, and env wins over the file hash", async () => {
  ...
  assert.equal(isValidBasicAuthorization(authorization("grok", "twelve chars!")), true);
  assert.equal(isValidBasicAuthorization(authorization("grok", "wrong password")), false);

  assert.equal(isValidBasicAuthorization(authorization("grok", "env-password-ok"), "env-password-ok"), true);
  assert.equal(isValidBasicAuthorization(authorization("grok", "twelve chars!"), "env-password-ok"), false);
});
