...
test("maps ACP fuzzy matches onto @-menu entries", async () => {
  const { mapAcpFuzzyMatches } = await loadSubject();
  assert.deepEqual(
    mapAcpFuzzyMatches([
      { name: "runtime.ts", type: "file", path: "/tmp/p/lib/runtime.ts" },
      { name: "lib", type: "directory", path: "/tmp/p/lib" },
    ], "/tmp/p"),
    [
      { path: "lib/runtime.ts", isDir: false },
      { path: "lib", isDir: true },
    ],
  );
});
