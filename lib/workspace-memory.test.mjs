...
test("clearing the last entry removes the storage key entirely", () => {
  ...
  clearLastOpen("root-a", storage);
  assert.equal(getLastOpenSession("root-a", storage), null);
  assert.equal(storage.values.has("grok-web:last-open-by-workspace"), false);
});

test("reads leftover pi-web workspace memory and writes the grok-web key", () => {
  const storage = createStorage({
    "pi-web:last-open-by-workspace": JSON.stringify({ "root-a": "session-legacy" }),
  });
  assert.equal(getLastOpenSession("root-a", storage), "session-legacy");
  setLastOpenSession("root-a", "session-next", storage);
  assert.equal(storage.values.has("pi-web:last-open-by-workspace"), false);
  assert.equal(getLastOpenSession("root-a", storage), "session-next");
});
...
test("workspaceKeyOf prefers projectRoot over cwd", () => {
