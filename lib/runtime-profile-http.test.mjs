...
test("GET exposes persisted-profile warnings without exposing filesystem paths", async () => {
  const handlers = createRuntimeProfileHandlers({
    readStatus: () => ({ profile: DEFAULT_RUNTIME_PROFILE, warnings: ["runtime profile ignored: invalid profile"] }),
  ...
  assert.deepEqual(body.warnings, ["runtime profile ignored: invalid profile"]);
});
