...
async function loadSubject() {
  try {
    ...
    return createJiti(import.meta.url, { alias: { "@": process.cwd() } }).import("./model-scope.ts");
  } catch {
...
test.skip("expands provider globs alongside exact references (#307)", async () => {
...
test.skip("accepts provider-qualified and unique bare exact model ids", async () => {
...
test.skip("matches bare model id globs across providers without duplicates", async () => {
...
test.skip("keeps thinking-level suffixes out of the matched reference and reports them as pins", async () => {
...
test.skip("leaves models without a pinned thinking level unpinned", async () => {
...
test.skip("reports patterns that match nothing but keeps the models that matched", async () => {
...
test.skip("falls back to all available models when nothing matches at all", async () => {
...
test.skip("selects the saved scoped default and applies its thinking pin", async () => {
...
test.skip("uses resolver order when the saved default is outside the enabled scope", async () => {
  const scope = await resolveVisibleModels(runtime, [
    "anthropic/claude-opus-5:high",
...
test.skip("applies a requested scoped model pin unless thinking was explicitly overridden", async () => {
...
test.skip("rejects an explicit model outside the enabled scope", async () => {
