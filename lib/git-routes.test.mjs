...
test("git routes use the authorized cwd instead of process-global ACP state", async (t) => {
  ...
  setAgentRuntime({
    ...
    gitStage: async (paths) => {
      acpCalls.push("stage");
      const rel = paths.map((item) => item === "nested.txt" ? "subdirectory/nested.txt" : item);
      await git(repositoryB, ["--literal-pathspecs", "add", "--", ...rel]);
      return { paths };
    },
    gitDiscard: async (paths) => {
      acpCalls.push("discard");
      await git(repositoryB, ["--literal-pathspecs", "restore", "--worktree", "--", ...paths]);
      return { ok: true };
    },
    gitCommit: async (message) => {
      acpCalls.push("commit");
      await git(repositoryB, ["commit", "-m", message]);
      return { ok: true };
    },
  });
  ...
  try {
    ...
    assert.ok(acpCalls.includes("stage"));
    assert.ok(acpCalls.includes("discard"));
    assert.ok(acpCalls.includes("commit"));
    assert.equal(acpCalls.includes("status"), false);
    assert.equal(acpCalls.includes("diff"), false);
  } finally {
    resetAgentRuntime();
...
test("git routes keep magic pathspecs inside an authorized subdirectory cwd", async (t) => {
  ...
  setAgentRuntime({
    ...
    gitDiscard: async (paths) => {
      await git(cwd, ["--literal-pathspecs", "restore", "--worktree", "--", ...paths]);
      return { ok: true };
    },
  });
  try {
