...
test("project trust HTTP uses AgentRuntime only, not rpc-manager", async () => {
  const source = await readFile(new URL("./project-trust-http.ts", import.meta.url), "utf8");
  assert.doesNotMatch(source, /rpc-manager/);
  assert.doesNotMatch(source, /hasBusyRpcSessionForCwd|destroyRpcSessionsForCwd/);
  assert.match(source, /getAgentRuntime\(\)\.hasBusySessionForCwd/);
  assert.match(source, /getAgentRuntime\(\)\.dropSessionsForCwd/);
});
...
test("project trust route reports resources and persists approval", async () => {
  ...
  try {
    const { GET, POST } = await jiti.import("./project-trust-http.ts");
...
test("project trust route keeps one canonical cwd if an alias is retargeted", async () => {
  ...
  try {
    const { POST } = await jiti.import("./project-trust-http.ts");
...
test("project trust API returns corrupt-store diagnostics with restricted status", async () => {
  ...
  try {
    const jiti = createJiti(import.meta.url, { alias: { "@": process.cwd() } });
    const { GET } = await jiti.import("./project-trust-http.ts");
