...
`);
  ...
  it("does not treat an empty commented api_key as official authentication", () => {
    const home = mkdtempSync(join(tmpdir(), "grok-empty-key-"));
    writeFileSync(join(home, "config.toml"), `api_key = "" # intentionally empty\n`);
    assert.equal(hasGrokApiKey(home), false);
  });
