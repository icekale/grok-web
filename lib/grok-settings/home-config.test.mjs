...
`);
  ...
  it("clears model-table grok api keys on disconnect without touching mcp_servers", () => {
    const home = mkdtempSync(join(tmpdir(), "grok-apikey-clear-model-"));
    writeFileSync(join(home, "config.toml"), `
api_key = "top-secret"
...
`);
    ...
    assert.equal(hasGrokApiKey(home), false);
    assert.doesNotMatch(after, /top-secret|model-secret/);
    assert.match(after, /api_key = "mcp-secret"/);
    assert.match(after, /\[model\."grok-4\.6"\]/);
  });

  it("clears OAuth tokens from auth.json", () => {
    const home = mkdtempSync(join(tmpdir(), "grok-auth-clear-"));
    writeFileSync(join(home, "auth.json"), JSON.stringify({ "grok.com": { token: "oauth" } }));
    assert.equal(readGrokAuth(home).loggedIn, true);
    clearGrokAuth(home);
    assert.deepEqual(readGrokAuth(home), { loggedIn: false, methods: [] });
  });
