...
test("RPC session startup opens an existing session file only once and trusts its cwd", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const routeSource = await readFile(new URL("../src/routes/api/agent/$id.ts", import.meta.url), "utf8");
  const eventRouteSource = await readFile(new URL("./agent-events-http.ts", import.meta.url), "utf8");
  const autoNameRouteSource = await readFile(new URL("./session-http.ts", import.meta.url), "utf8");
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const deleteRouteSource = await readFile(new URL("../src/routes/api/sessions/$id.ts", import.meta.url), "utf8");
  const trustRouteSource = await readFile(new URL("./project-trust-http.ts", import.meta.url), "utf8");
  ...
  assert.match(deleteRouteSource, /deleteSession\(params\.id\)/);
  assert.match(trustRouteSource, /await destroyRpcSessionsForCwd\(result\.cwd\)/);
  assert.match(trustRouteSource, /hasBusySessionForCwd/);
  assert.match(trustRouteSource, /dropSessionsForCwd/);
});

test("new-session route applies model scope during construction instead of follow-up commands", async () => {
  const route = await readFile(new URL("../src/routes/api/agent/new.ts", import.meta.url), "utf8");
  const source = await readFile(new URL("./acp/http.ts", import.meta.url), "utf8");

  assert.match(route, /createAgentHandlers\(getAgentRuntime\(\)\)\.postNew/);
  assert.match(source, /state\.thinkingLevel/);
  assert.doesNotMatch(source, /type: "set_model"/);
  assert.doesNotMatch(source, /type: "set_thinking_level"/);
  assert.match(source, /state\.model/);
});

test("prompt routes mark only preflight failures as rejected", async () => {
  const existingRoute = await readFile(new URL("../src/routes/api/agent/$id.ts", import.meta.url), "utf8");
  const newRoute = await readFile(new URL("../src/routes/api/agent/new.ts", import.meta.url), "utf8");
  const source = await readFile(new URL("./acp/http.ts", import.meta.url), "utf8");

  assert.match(existingRoute, /createAgentHandlers\(getAgentRuntime\(\)\)\.postSession/);
  assert.match(newRoute, /createAgentHandlers\(getAgentRuntime\(\)\)\.postNew/);
  assert.match(source, /let promptAccepted = false/);
  assert.match(source, /await .*\.send\(/);
  assert.match(source, /promptAccepted = true/);
  assert.match(source, /commandType === "prompt" && !extras.promptAccepted/);
});
...
test("saving model config refreshes only existing live sessions without creating a registry", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const routeSource = await readFile(new URL("./models-config-http.ts", import.meta.url), "utf8");
  ...
  assert.doesNotMatch(routeSource, /refreshRpcSessionModelConfigs|rpc-manager/);
});
...
test("shutdown flushes a running turn before disposing", async () => {
  ...
  assert.match(shutdownSource, /if \(this\.isRunning\(\)\)/);
  assert.ok(
...
test("SIGTERM cleans sessions once and then terminates with the signal", async (t) => {
  const child = spawn(process.execPath, ["--input-type=module", "--eval", `
    ...
    const { getRpcSession } = await jiti.import("./lib/rpc-manager.ts");
    getRpcSession("initialize-registry");
    globalThis.__piSessions.set("throws", {
      destroy() { throw new Error("destroy failed"); },
    });
    globalThis.__piSessions.set("tracked", {
      destroy() { process.stdout.write("cleaned\\n"); },
    });
  ...
  await Promise.race([
    ...
    new Promise((_, reject) => setTimeout(() => reject(new Error("child did not become ready")), 10_000)),
  ]);
  ...
  assert.equal((stdout.match(/cleaned/g) ?? []).length, 1);
});
