...
test("request security requires Basic Auth on non-loopback hosts when a password is set", async () => {
  ...
  try {
    const remote = getRequestSecurityRejection(new Request("http://app.example.com/api/sessions", {
    ...
    const loopback = getRequestSecurityRejection(new Request("http://127.0.0.1:30142/api/sessions", {
