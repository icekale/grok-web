...
test("the CLI refuses a non-loopback bind when GROK_WEB_PASSWORD is shorter than 12 characters", () => {
  const fixture = buildFakePackage();
  const result = runCli(fixture, ["--no-open", "-H", "0.0.0.0", "-p", "30225"], {
    GROK_WEB_PASSWORD: "short",
  });
  assert.equal(result.status, 1, result.stderr);
  assert.match(result.stderr, /password|authentication/i);
  assert.doesNotMatch(result.stdout, /Listening/);
