...
describe("GET /api/sessions", () => {
  ...
  it("DELETE removes an idle session after ACP load and close", async () => {
    ...
    try {
      ...
      const res = await deleteSession(id, {
        ...
        closeSession: async (sessionId) => { calls.push(`close:${sessionId}`); return { outcome: "closed" }; },
      });
      assert.deepEqual(calls, [`load:${id}`, `close:${id}`]);
      assert.equal(res.status, 200);
  ...
  it("DELETE refuses a busy owned session without touching disk", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-api-del-busy-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const id = "01bbbbbbbbbbbbbbbbbbbbbb";
      const dir = join(home, "sessions", encodeURIComponent("/tmp/p"), id);
      await mkdir(dir, { recursive: true });
      await writeFile(join(dir, "summary.json"), JSON.stringify({ info: { id, cwd: "/tmp/p" } }));
      const response = await deleteSession(id, {
        hasSession: () => true,
        isBusy: () => true,
        loadSession: async () => { throw new Error("must not load"); },
        closeSession: async () => { throw new Error("must not close"); },
      });
      assert.equal(response.status, 409);
      assert.equal((await response.json()).code, "session_busy");
  ...
  it("DELETE refuses when ACP cannot load the session", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-api-del-owned-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const id = "01cccccccccccccccccccccc";
      const dir = join(home, "sessions", encodeURIComponent("/tmp/p"), id);
      await mkdir(dir, { recursive: true });
      await writeFile(join(dir, "summary.json"), JSON.stringify({ info: { id, cwd: "/tmp/p" } }));
      const response = await deleteSession(id, {
  ...
  it("DELETE removes an unloadable leftover session without calling close", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-api-del-orphan-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const id = "01oooooooooooooooooooooo";
      const dir = join(home, "sessions", encodeURIComponent("/tmp"), id);
      await mkdir(dir, { recursive: true });
      await writeFile(join(dir, "summary.json"), JSON.stringify({
        info: { id, cwd: "/tmp" },
        session_summary: "",
      ...
      const response = await deleteSession(id, {
        hasSession: () => false,
        isBusy: () => false,
        loadSession: async () => { throw new Error("Path not found."); },
        closeSession: async () => { calls.push("close"); return { outcome: "closed" }; },
      });
  ...
  it("DELETE refuses when ACP close fails", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-api-del-close-fail-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const id = "01dddddddddddddddddddddd";
      const dir = join(home, "sessions", encodeURIComponent("/tmp/p"), id);
      await mkdir(dir, { recursive: true });
      await writeFile(join(dir, "summary.json"), JSON.stringify({ info: { id, cwd: "/tmp/p" } }));
      const response = await deleteSession(id, {
  ...
  it("DELETE refuses a non-closed ACP outcome", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-api-del-outcome-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const id = "01ffffffffffffffffffffff";
      const dir = join(home, "sessions", encodeURIComponent("/tmp/p"), id);
      await mkdir(dir, { recursive: true });
      await writeFile(join(dir, "summary.json"), JSON.stringify({ info: { id, cwd: "/tmp/p" } }));
      const response = await deleteSession(id, {
        ...
        isBusy: () => false,
        loadSession: async () => { throw new Error("must not load"); },
        closeSession: async () => ({ outcome: "failed" }),
      });
      assert.equal(response.status, 409);
      assert.equal((await response.json()).code, "session_in_use");
      assert.equal(existsSync(dir), true);
  ...
  it("DELETE closes an already-owned idle session before removal", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-api-del-idle-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const id = "01eeeeeeeeeeeeeeeeeeeeee";
      const dir = join(home, "sessions", encodeURIComponent("/tmp/p"), id);
      await mkdir(dir, { recursive: true });
      await writeFile(join(dir, "summary.json"), JSON.stringify({ info: { id, cwd: "/tmp/p" } }));
      const calls = [];
      const response = await deleteSession(id, {
        hasSession: () => true,
        isBusy: () => false,
        loadSession: async () => calls.push("load"),
        closeSession: async () => { calls.push("close"); return { outcome: "closed" }; },
      });
      assert.equal(response.status, 200);
  ...
  it("GET state does not start a Pi SessionManager and returns ACP or idle state", async () => {
    ...
    try {
      ...
      assert.equal(body.state.thinkingLevel, undefined);
      assert.deepEqual(body.state.queuedMessages, { steering: [], followUp: [] });
      assert.deepEqual(body.state.contextUsage, {
        percent: 6,
        tokens: 33669,
...
describe("GET /api/sessions/:id", () => {
  it("maps updates.jsonl into session detail context.messages", async () => {
    ...
    try {
      ...
      assert.deepEqual(body.context.messages[1].content, [{ type: "text", text: "Yo" }]);
      assert.deepEqual(body.context.entryIds, ["e1", "e2"]);
      assert.equal(body.context.thinkingLevel, "xhigh");
    } finally {
      if (prev === undefined) delete process.env.GROK_HOME;
      else process.env.GROK_HOME = prev;
    }
