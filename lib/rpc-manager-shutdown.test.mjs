...
test("queue-wide partial failure preserves duplicate messages by index", async (t) => {
  const queue = {
    steering: ["duplicate", "duplicate", "tail"],
    followUp: ["duplicate"],
  };
  let steerCalls = 0;
  const inner = {
    ...
    clearQueue: () => {
      const snapshot = {
        steering: queue.steering.splice(0),
        followUp: queue.followUp.splice(0),
      };
      return snapshot;
    },
    steer: async (text) => {
      steerCalls += 1;
      if (steerCalls === 2) throw new Error("second delivery failed");
      if (steerCalls > 2) queue.steering.push(text);
    },
    followUp: async (text) => {
  ...
  await assert.rejects(
    wrapper.send({ type: "queue_steer_all" }),
    /second delivery failed/,
  );

  assert.deepEqual(queue, {
    steering: ["duplicate", "tail"],
    followUp: ["duplicate"],
  });
});

test("queue mutations serialize clear and rebuild operations", async (t) => {
  const queue = { steering: [], followUp: ["a", "b"] };
  let clearCalls = 0;
  let releaseFirstRequeue;
  const firstRequeue = new Promise((resolve) => { releaseFirstRequeue = resolve; });
  let followUpCalls = 0;
  const inner = {
    isBashRunning: false,
    extensionRunner: {},
    ...
    followUp: async (text) => {
      followUpCalls += 1;
      if (followUpCalls === 1) await firstRequeue;
      queue.followUp.push(text);
    },
    dispose() {},
  };
  ...
  const remove = wrapper.send({
    type: "queue_remove",
    kind: "followUp",
    text: "a",
  });
  await nextTurn();
  const edit = wrapper.send({
    type: "queue_edit",
    kind: "followUp",
    text: "b",
    replacement: "B",
  ...
  assert.deepEqual(queue.followUp, ["B"]);
});
...
test.skip("direct bash commands use sanitized project operations with current shell settings", async (t) => {
  let received;
