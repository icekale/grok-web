...
const pwa = await readFile(new URL("../components/PwaRegistration.tsx", import.meta.url), "utf8");
...
test("Vite defines the two existing public version variables", () => {
  assert.match(vite, /__GROK_WEB_APP_VERSION__/);
  assert.match(vite, /__GROK_WEB_BUILD_ID__/);
  assert.match(vite, /process\.env\.NEXT_PUBLIC_APP_VERSION/);
  assert.match(vite, /process\.env\.NEXT_PUBLIC_BUILD_ID/);
  assert.match(vite, /process\.env\.NEXT_PUBLIC_PI_VERSION/);
  assert.match(pwa, /process\.env\.NEXT_PUBLIC_BUILD_ID/);
});
