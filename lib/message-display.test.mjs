...
test("skips missing content blocks instead of crashing on .type", async () => {
  const { getDisplayableAssistantBlocks, splitFinalAssistantBlocks, countToolCallBlocks } = await loadSubject();
  const message = assistant([
    undefined,
    { type: "thinking", thinking: "" },
    { type: "text", text: "ok" },
  ...
  assert.equal(splitFinalAssistantBlocks(message).answerBlocks[0].text, "ok");
  assert.equal(countToolCallBlocks(blocks), 0);
});

test("detects aborted assistant turns after the stream ends", async () => {
  const { isAbortedAssistantMessage, getAssistantAbortDetail } = await loadSubject();
  const aborted = { ...assistant([]), stopReason: "aborted" };

  assert.equal(isAbortedAssistantMessage(aborted), true);
  assert.equal(isAbortedAssistantMessage(aborted, { isStreaming: true }), false);
  assert.equal(isAbortedAssistantMessage({ ...assistant([]), stopReason: "stop" }), false);
  assert.equal(getAssistantAbortDetail(aborted), null);
  assert.equal(
    getAssistantAbortDetail({ ...aborted, errorMessage: "Request was aborted" }),
    null,
  );
  assert.equal(
    getAssistantAbortDetail({ ...aborted, errorMessage: "Aborted after 2 retry attempts" }),
