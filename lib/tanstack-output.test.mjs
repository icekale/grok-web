...
test("Windows gate builds and runs the generated server on Windows", async (t) => {
  const workflowUrl = new URL("../.github/workflows/tanstack-spike-windows.yml", import.meta.url);
  let workflow;
  try {
    workflow = await readFile(workflowUrl, "utf8");
  } catch (error) {
    if (error?.code === "ENOENT") {
      t.skip("Windows TanStack workflow is not in this repo");
      return;
    }
    throw error;
  }
