...
test("connection test rejects public hostnames resolving to link-local", async () => {
  const { runVisionToolkitHealth } = await loadHelper();
  let fetches = 0;
  const result = await runVisionToolkitHealth({
    testConnection: true,
    ...
    ...localStubs({ lookup: async () => [{ address: "169.254.169.254", family: 4 }] }),
  });

  assert.equal(fetches, 0);
  assert.equal(result.connectionTested, true);
  assert.equal(result.checks.service.status, "error");
  assert.match(result.checks.service.detail, /link-local|special-use/i);
});
