...
test("TanStack /api/plugins serves the MCP HTTP handlers, not grok plugins-http", async () => {
  const source = await readFile(new URL("../src/routes/api/plugins.ts", import.meta.url), "utf8");
  assert.match(source, /from ["']@\/lib\/mcp-http["']/);
  assert.doesNotMatch(source, /plugins-http/);
});
...
describe("/api/mcp adapter", () => {
  ...
  it("GET /api/plugins uses the same MCP package list as /api/mcp", async () => {
    setAgentRuntime(createRuntime());
