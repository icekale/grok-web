import assert from "node:assert/strict";
import { mkdtemp, mkdir, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { describe, it } from "node:test";
import { findGrokSession, listGrokSessions } from "./session-index.ts";

async function writeSummary(dir, body) {
  await mkdir(dir, { recursive: true });
  await writeFile(join(dir, "summary.json"), JSON.stringify(body));
}

describe("listGrokSessions", () => {
  it("groups encoded cwd and .cwd fallback, skips corrupt sessions", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-idx-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const id1 = "01aaaaaaaaaaaaaaaaaaaaaaaa";
      const id2 = "01bbbbbbbbbbbbbbbbbbbbbbbb";
      const id3 = "01cccccccccccccccccccccccc";
      await writeSummary(join(home, "sessions", encodeURIComponent("/tmp/demo"), id1), {
        info: { id: id1, cwd: "/tmp/demo" },
        session_summary: "Fix login",
        created_at: "2026-08-18T13:00:00.000Z",
        updated_at: "2026-08-18T13:10:00.000Z",
        last_active_at: "2026-08-18T13:20:00.000Z",
        num_chat_messages: 4,
        generated_title: "Fix login bug",
      });
      const slug = join(home, "sessions", "too-long-slug");
      await mkdir(slug, { recursive: true });
      await writeFile(join(slug, ".cwd"), "/very/long/project\n");
      await writeSummary(join(slug, id2), {
        info: { id: id2, cwd: "/ignored" },
        session_summary: "Other",
        created_at: "2026-08-18T12:00:00.000Z",
        updated_at: "2026-08-18T12:00:00.000Z",
        num_messages: 2,
      });
      await mkdir(join(home, "sessions", encodeURIComponent("/tmp/bad"), id3), { recursive: true });
      await writeFile(join(home, "sessions", encodeURIComponent("/tmp/bad"), id3, "summary.json"), "{");

      const sessions = await listGrokSessions();
      assert.equal(sessions.length, 2);
      assert.equal(sessions[0].id, id1);
      assert.equal(sessions[0].cwd, "/tmp/demo");
      assert.equal(sessions[0].name, "Fix login bug");
      assert.equal(sessions[0].modified, "2026-08-18T13:20:00.000Z");
      assert.equal(sessions[0].messageCount, 4);
      assert.equal(sessions[0].firstMessage, "Fix login");
      assert.equal(sessions[0].path, join(home, "sessions", encodeURIComponent("/tmp/demo"), id1));
      assert.equal(sessions[1].id, id2);
      assert.equal(sessions[1].cwd, "/very/long/project");
      assert.equal(sessions[1].name, "Other");
    } finally {
      if (prev === undefined) delete process.env.GROK_HOME;
      else process.env.GROK_HOME = prev;
    }
  });

  it("skips unnamed zero-message tool sessions but keeps named sessions", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-idx-empty-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const emptyId = "01iiiiiiiiiiiiiiiiiiiiiiii";
      const namedId = "01jjjjjjjjjjjjjjjjjjjjjjjj";
      const historyId = "01kkkkkkkkkkkkkkkkkkkkkkkk";
      const legacyId = "01llllllllllllllllllllllll";
      await writeSummary(join(home, "sessions", encodeURIComponent("/tmp/tool-only"), emptyId), {
        info: { id: emptyId, cwd: "/tmp/tool-only" },
        session_summary: "",
        generated_title: "",
        created_at: "2026-08-22T00:00:00.000Z",
        updated_at: "2026-08-22T00:00:00.000Z",
        num_messages: 0,
        num_chat_messages: 2,
      });
      await writeSummary(join(home, "sessions", encodeURIComponent("/tmp/named"), namedId), {
        info: { id: namedId, cwd: "/tmp/named" },
        session_summary: "",
        generated_title: "Keep this draft",
        created_at: "2026-08-22T00:00:00.000Z",
        updated_at: "2026-08-22T00:00:00.000Z",
        num_messages: 0,
        num_chat_messages: 2,
      });
      const historyDir = join(home, "sessions", encodeURIComponent("/tmp/history"), historyId);
      await writeSummary(historyDir, {
        info: { id: historyId, cwd: "/tmp/history" },
        session_summary: "",
        generated_title: "",
        created_at: "2026-08-22T00:00:00.000Z",
        updated_at: "2026-08-22T00:00:00.000Z",
        num_messages: 0,
      });
      await writeFile(join(historyDir, "updates.jsonl"), `${JSON.stringify({
        method: "session/update",
        params: {
          update: { sessionUpdate: "user_message_chunk", content: { type: "text", text: "Keep recovered history" } },
        },
      })}\n`);
      await writeSummary(join(home, "sessions", encodeURIComponent("/tmp/legacy"), legacyId), {
        info: { id: legacyId, cwd: "/tmp/legacy" },
        session_summary: "",
        generated_title: "",
        created_at: "2026-08-22T00:00:00.000Z",
        updated_at: "2026-08-22T00:00:00.000Z",
      });

      const sessions = await listGrokSessions();
      assert.deepEqual(new Set(sessions.map((session) => session.id)), new Set([namedId, historyId, legacyId]));
      assert.equal(sessions.find((session) => session.id === namedId)?.name, "Keep this draft");
      assert.equal(sessions.find((session) => session.id === historyId)?.name, "Keep recovered history");
      assert.equal(sessions.find((session) => session.id === legacyId)?.firstMessage, "(no messages)");
    } finally {
      if (prev === undefined) delete process.env.GROK_HOME;
      else process.env.GROK_HOME = prev;
    }
  });

  it("marks session_kind subagent and subagent_resume as sessionRole subagent", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-idx-sub-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const primary = "01eeeeeeeeeeeeeeeeeeeeeeee";
      const child = "01ffffffffffffffffffffffff";
      const resumed = "01gggggggggggggggggggggggg";
      await writeSummary(join(home, "sessions", encodeURIComponent("/tmp/demo"), primary), {
        info: { id: primary, cwd: "/tmp/demo" },
        session_summary: "Parent",
        created_at: "2026-08-19T00:00:00.000Z",
        updated_at: "2026-08-19T00:00:00.000Z",
        generated_title: "Parent",
      });
      await writeSummary(join(home, "sessions", encodeURIComponent("/tmp/wt"), child), {
        info: { id: child, cwd: "/tmp/wt" },
        session_kind: "subagent",
        session_summary: "Child",
        created_at: "2026-08-19T00:00:00.000Z",
        updated_at: "2026-08-19T00:00:00.000Z",
        generated_title: "Child",
      });
      await writeSummary(join(home, "sessions", encodeURIComponent("/tmp/demo"), resumed), {
        info: { id: resumed, cwd: "/tmp/demo" },
        session_kind: "subagent_resume",
        parent_session_id: primary,
        session_summary: "Resumed",
        created_at: "2026-08-19T00:00:00.000Z",
        updated_at: "2026-08-19T00:00:00.000Z",
        generated_title: "Resumed",
      });
      const sessions = await listGrokSessions();
      assert.notEqual(sessions.find((session) => session.id === primary)?.sessionRole, "subagent");
      assert.equal(sessions.find((session) => session.id === child)?.sessionRole, "subagent");
      assert.equal(sessions.find((session) => session.id === resumed)?.sessionRole, "subagent");
    } finally {
      if (prev === undefined) delete process.env.GROK_HOME;
      else process.env.GROK_HOME = prev;
    }
  });
});

describe("findGrokSession", () => {
  it("returns a session by exact id or null", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-idx-find-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const id = "01dddddddddddddddddddddddd";
      await writeSummary(join(home, "sessions", encodeURIComponent("/tmp/demo"), id), {
        info: { id, cwd: "/tmp/demo" },
        session_summary: "Find me",
        created_at: "2026-08-18T13:00:00.000Z",
        updated_at: "2026-08-18T13:10:00.000Z",
        num_chat_messages: 1,
        generated_title: "Found",
      });
      const found = await findGrokSession(id);
      assert.equal(found?.id, id);
      assert.equal(found?.name, "Found");
      assert.equal(await findGrokSession("missing"), null);
    } finally {
      if (prev === undefined) delete process.env.GROK_HOME;
      else process.env.GROK_HOME = prev;
    }
  });

  it("falls back to the first user message when Grok left session_summary empty", async () => {
    const home = await mkdtemp(join(tmpdir(), "grok-idx-fallback-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    try {
      const id = "01hhhhhhhhhhhhhhhhhhhhhhhh";
      const dir = join(home, "sessions", encodeURIComponent("/tmp/demo"), id);
      await writeSummary(dir, {
        info: { id, cwd: "/tmp/demo" },
        session_summary: "",
        created_at: "2026-08-21T00:00:00.000Z",
        updated_at: "2026-08-21T00:00:00.000Z",
        num_chat_messages: 2,
      });
      await writeFile(join(dir, "updates.jsonl"), `${JSON.stringify({
        timestamp: 1,
        method: "session/update",
        params: {
          sessionId: id,
          update: { sessionUpdate: "user_message_chunk", content: { type: "text", text: "  Fix the rename  fallback\n" } },
          _meta: { eventId: "e1" },
        },
      })}\n`);
      const found = await findGrokSession(id);
      assert.equal(found?.firstMessage, "Fix the rename fallback");
      assert.equal(found?.name, "Fix the rename fallback");
    } finally {
      if (prev === undefined) delete process.env.GROK_HOME;
      else process.env.GROK_HOME = prev;
    }
  });
});
