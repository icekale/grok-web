...
function deleteRequest(body) {
  return new Request("http://127.0.0.1/api/worktrees", {
    ...
    body: JSON.stringify({ sessionId: "sess-1", ...body }),
  });
}
...
test("DELETE only removes an authorized linked worktree from the cwd repository", async (t) => {
  ...
  setAgentRuntime({
    hasSession: () => true,
    ensureProcess: async () => {},
    worktreeRemove: async (target, force = false) => {
      acpCalls.push(target);
      await git(repositoryB.repository, ["worktree", "remove", ...(force ? ["--force"] : []), target]);
      return { ok: true };
    },
  });

  try {
    const { DELETE } = await jiti.import("./worktrees-http.ts");
