...
test("corrupt and non-object models.json fail closed instead of rewriting empty config", (t) => {
  const root = createTempRoot(t);
  const modelsPath = join(root, "models.json");
  writeFileSync(modelsPath, "[1, 2]");
  assert.throws(() => readModelsConfig(modelsPath), ModelsConfigError);
  assert.equal(readFileSync(modelsPath, "utf8"), "[1, 2]");
  writeFileSync(modelsPath, "{not json");
  assert.throws(() => readModelsConfig(modelsPath), /not valid JSON/);
  assert.equal(readFileSync(modelsPath, "utf8"), "{not json");
  assert.throws(() => writeModelsConfig([], modelsPath), /must be an object/);
});
...
test.skip("an existing session opens after its historical model is removed from config", (t) => {
  const root = createTempRoot(t);
