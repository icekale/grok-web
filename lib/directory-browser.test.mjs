...
test("creates a direct child directory and returns its resolved path", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "pi-web-create-folder-"));
  try {
    const { createChildDirectory } = await loadSubject();
    const created = await createChildDirectory(root, " new-project ");

    assert.equal(created, path.join(await realpath(root), "new-project"));
...
test("rejects empty, traversing, separated, and reserved folder names", async () => {
  const { validateDirectoryName } = await loadSubject();

  for (const name of ["", "  ", ".", "..", "child/name", "child\\name", "CON", "trailing."]) {
    assert.throws(() => validateDirectoryName(name), TypeError);
  }
  assert.equal(validateDirectoryName("project.v2"), "project.v2");
});

test("does not overwrite an existing directory", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "pi-web-create-conflict-"));
  try {
    await mkdir(path.join(root, "existing"));
    const { createChildDirectory } = await loadSubject();

    await assert.rejects(
      createChildDirectory(root, "existing"),
