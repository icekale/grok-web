...
const EXPECTED_ADAPTERS = {
  ...
  "src/routes/api/git/commit.ts": { route: "/api/git/commit", methods: ["POST"] },
  "src/routes/api/git/diff.ts": { route: "/api/git/diff", methods: ["GET"] },
  "src/routes/api/git/discard.ts": { route: "/api/git/discard", methods: ["POST"] },
  "src/routes/api/git/stage.ts": { route: "/api/git/stage", methods: ["POST"] },
  "src/routes/api/git/status.ts": { route: "/api/git/status", methods: ["GET"] },
  "src/routes/api/hooks.ts": { route: "/api/hooks", methods: ["GET", "POST"] },
  "src/routes/api/memory.ts": { route: "/api/memory", methods: ["GET", "POST"] },
  "src/routes/api/meta.ts": { route: "/api/meta", methods: ["GET", "POST"] },
  ...
  "src/routes/api/mcp.ts": { route: "/api/mcp", methods: ["GET", "POST"] },
  ...
  "src/routes/api/runtime-profile.ts": { route: "/api/runtime-profile", methods: ["GET", "PUT"] },
  "src/routes/api/settings.ts": { route: "/api/settings", methods: ["GET", "PUT"] },
  ...
  "src/routes/api/sessions/$id/restore-code.ts": { route: "/api/sessions/$id/restore-code", methods: ["POST"] },
  ...
  "src/routes/api/vision-toolkit.ts": { route: "/api/vision-toolkit", methods: ["GET", "PUT"] },
  "src/routes/api/vision-toolkit/health.ts": { route: "/api/vision-toolkit/health", methods: ["POST"] },
  "src/routes/api/vision-toolkit/reveal.ts": { route: "/api/vision-toolkit/reveal", methods: ["POST"] },
...
test("every API handler has a thin TanStack adapter with the expected route and methods", async () => {
  assert.equal(Object.keys(EXPECTED_ADAPTERS).length, 55);
...
test("app/api has no route.ts HTTP handlers", async () => {
  const apiDir = join(ROOT, "app", "api");
  try {
    const actual = await filesNamedRoute(apiDir);
...
test("TanStack API adapters import lib handlers and use standard Web APIs", async () => {
  for (const file of Object.keys(EXPECTED_ADAPTERS)) {
    const source = await readFile(join(ROOT, file), "utf8");
    assert.doesNotMatch(source, /from ["']next\/server["']/, file);
    assert.doesNotMatch(source, /\bNextRequest\b|\bNextResponse\b|\.nextUrl\b/, file);
    assert.doesNotMatch(source, /from ["']@\/app\/api/, file);
    assert.match(source, /from ["']@\/lib\//, file);
  }
});
