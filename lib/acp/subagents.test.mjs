...
describe("grok subagent adapter", () => {
  ...
  it("finds a child by either subagentId or childSessionId and interrupts the real subagentId", async () => {
    const metas = [{
      subagentId: "run-abc",
      parentSessionId: "root",
    ...
    const byChild = findGrokChild("root", "sess-xyz", [session("root")], metas);
    const byRun = findGrokChild("root", "run-abc", [session("root")], metas);
    assert.equal(byChild?.id, "sess-xyz");
    assert.equal(byChild?.subagentRunId, "run-abc");
    assert.equal(byRun?.subagentRunId, "run-abc");
    const cancelled = [];
    await controlGrokSubagent(
      { send: async () => ({}), cancelSubagent: async (id) => cancelled.push(id) },
      "root",
      "sess-xyz",
      "interrupt",
      undefined,
      { subagentId: byChild?.subagentRunId },
    );
    assert.deepEqual(cancelled, ["run-abc"]);
  });

  it("rejects blank steer and resume messages", async () => {
    const runtime = { send: async () => ({}) };
    await assert.rejects(() => controlGrokSubagent(runtime, "root", "child", "steer", "   "), /non-empty message/);
    await assert.rejects(() => controlGrokSubagent(runtime, "root", "child", "steer"), /non-empty message/);
    await assert.rejects(
      () => controlGrokSubagent({ ...runtime, resumeSession: async () => {} }, "root", "child", "resume", ""),
      /non-empty message/,
    );
  });
