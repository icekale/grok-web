...
import { resolvedSessionModelId, sessionModelRef } from "../grok-model-label.ts";
...
describe("mapGrokModels", () => {
  ...
  it("namespaced session models keep their Settings provider", () => {
    assert.deepEqual(sessionModelRef("cpa/grok-4.6"), { provider: "cpa", id: "cpa/grok-4.6" });
    assert.deepEqual(sessionModelRef("grok-4.6"), { provider: "grok", id: "grok-4.6" });
    assert.equal(resolvedSessionModelId("cpa/grok-4.6", "grok-4.6"), "cpa/grok-4.6");
    assert.equal(resolvedSessionModelId("grok-4.5", "grok-4.5"), "grok-4.5");
  });

  it("does not treat a selected permission mode as reasoning effort", () => {
    assert.equal(selectedGrokEffort({
      _meta: {
        "x.ai/sessionConfig": {
          options: [
            { id: "plan", category: "mode", selected: true },
