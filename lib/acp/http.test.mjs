...
test("GET and POST agent map TUI-owned sessions to 409 session_in_use", async () => {
  const home = await mkdtemp(join(tmpdir(), "grok-http-owned-"));
  const previousHome = process.env.GROK_HOME;
  ...
  const runtime = new AgentRuntime({
    connect: async () => ({
      initialize: async () => ({}),
      sessionLoad: async () => {
        throw new Error("already loaded elsewhere");
      },
      onSessionUpdate: () => () => {},
      onPermission: () => () => {},
    }),
  ...
  const handlers = createAgentHandlers(runtime);
  try {
    const getRes = await handlers.getSession(new Request(`http://127.0.0.1/api/agent/${id}`), id);
    assert.equal(getRes.status, 409);
    assert.equal((await getRes.json()).code, "session_in_use");
    const postRes = await handlers.postSession(new Request(`http://127.0.0.1/api/agent/${id}`, {
      method: "POST",
      headers: { "content-type": "application/json" },
    ...
    assert.equal(postBody.code, "session_in_use");
    assert.notEqual(postBody.error, "Session not found");
  } finally {
    if (previousHome === undefined) delete process.env.GROK_HOME;
    else process.env.GROK_HOME = previousHome;
  }
