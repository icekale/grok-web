...
describe("AgentRuntime", () => {
  ...
  it("passes reasoning effort when selecting and re-selecting a model", async () => {
    const models = [];
    const runtime = new AgentRuntime({
      connect: async () => ({
        initialize: async () => ({}),
    ...
    assert.deepEqual(models, [["grok-4.6", "xhigh"], ["cpa/grok-4.6", "xhigh"]]);
  });
  ...
  it("sends every thinking level in set_model metadata and preserves state on failure", async () => {
    ...
    assert.deepEqual(efforts, ["high", "off"]);
