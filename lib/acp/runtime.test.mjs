...
describe("AgentRuntime", () => {
  ...
  it("lists enabled skills as slash commands", async () => {
    const runtime = createRuntime();
    const sessionId = await runtime.createSession("/tmp/p");
    const listed = await runtime.send(sessionId, { type: "get_commands" });
    assert.ok(Array.isArray(listed.commands));
    const demo = listed.commands.find((command) => command.name === "demo");
    assert.equal(demo?.source, "skill");
    assert.equal(demo?.description, "demo skill");
    assert.equal(demo?.sourceInfo?.path, "/tmp/demo/SKILL.md");
  ...
  it("bash is an explicit unsupported error", async () => {
    const runtime = createRuntime();
    const sessionId = await runtime.createSession("/tmp/p");
    await assert.rejects(
      runtime.send(sessionId, { type: "bash", command: "ls", excludeFromContext: false }),
      /Shell commands are not supported/,
    );
  });

  it("abort_bash cancels the ACP session", async () => {
    const cancelled = [];
    const runtime = new AgentRuntime({
      connect: async () => ({
        initialize: async () => ({}),
  ...
  it("extension_ui_input and run_command are explicit unsupported errors", async () => {
    const runtime = createRuntime();
    const sessionId = await runtime.createSession("/tmp/p");
    await assert.rejects(
    ...
    await assert.rejects(
      runtime.send(sessionId, { type: "run_command", name: "todos-toggle" }),
      /Extension commands are not supported/,
    );
  });
