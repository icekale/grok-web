...
import { AgentRuntime, getAgentRuntime, resetAgentRuntime, setAgentRuntime } from "./runtime.ts";
...
describe("AgentRuntime", () => {
  ...
  it("reapplies the session effort before each prompt", async () => {
    ...
    const sessionId = await runtime.createSession("/tmp/p");
    await runtime.send(sessionId, { type: "prompt", message: "Hi" });
    assert.deepEqual(calls.at(-2), ["model", "grok-4.6", "high"]);
  ...
  it("createSession uses official high for grok-4.6 and clamps xhigh off grok-4.5", async () => {
    const models = [];
    const runtime = new AgentRuntime({
      connect: async () => ({
        initialize: async () => ({}),
  ...
  it("set_model does not reuse the permission mode RPC for reasoning effort", async () => {
    ...
    assert.deepEqual(models, [["grok-4.6", "high"], ["grok-4.6", "xhigh"], ["cpa/grok-4.6", "xhigh"]]);
    assert.equal((await runtime.send(sessionId, { type: "get_state" })).thinkingLevel, "xhigh");
  });
  ...
  it("sends every thinking level in set_model metadata and preserves state on failure", async () => {
    ...
    assert.deepEqual(efforts, ["high", "high", "off"]);
  ...
  it("passes the selected effort when re-applying the current model", async () => {
    ...
    assert.deepEqual(models, [["grok-4.6", "high"], ["grok-4.6", "xhigh"]]);
    assert.equal((await runtime.send(sessionId, { type: "get_state" })).thinkingLevel, "xhigh");
  });

  it("keeps set_thinking_level after a live recycle instead of reloading persisted ACP effort", async () => {
    ...
    await runtime.send(sessionId, { type: "set_thinking_level", level: "xhigh" });
    assert.ok(starts() >= 2, "effort change must recycle the live child");
    assert.equal((await runtime.send(sessionId, { type: "get_state" })).thinkingLevel, "xhigh");
  });
  ...
  it("does not commit spawn effort when another session is busy", async () => {
    let n = 0;
    let release;
    ...
    await new Promise((resolve) => setTimeout(resolve, 20));
    await runtime.send(idleId, { type: "set_thinking_level", level: "xhigh" });
    assert.equal(starts, started);
    release();
    await held;
    await runtime.send(idleId, { type: "set_thinking_level", level: "xhigh" });
    assert.ok(starts > started, "idle retry must recycle after the busy session settles");
    assert.equal((await runtime.send(idleId, { type: "get_state" })).thinkingLevel, "xhigh");
  });

  it("recycles when set_model moves from no-effort onto a grok-4.6 spawn fallback", async () => {
    const { runtime, starts } = liveEffortRuntime({
    ...
    const started = starts();
    await runtime.send(sessionId, { type: "set_model", provider: "cpa", modelId: "cpa/grok-4.6" });
    assert.ok(starts() > started, "model switch must recycle when spawn effort changes");
    assert.equal((await runtime.send(sessionId, { type: "get_state" })).thinkingLevel, "xhigh");
  });
