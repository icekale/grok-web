...
import { AgentCommandError, AgentRuntime, extraAcpReadRoots, getAgentRuntime, resetAgentRuntime, setAgentRuntime } from "./runtime.ts";
...
describe("AgentRuntime", () => {
  ...
  it("get_session_stats sums Grok turn usage from updates.jsonl", async () => {
    const runtime = createRuntime();
    const sessionId = "01cache-usage-aaaaaaaaaaaaaa";
    const dir = join(process.env.GROK_HOME, "sessions", encodeURIComponent("/tmp/cache"), sessionId);
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, "summary.json"), JSON.stringify({
      info: { id: sessionId, cwd: "/tmp/cache" },
      created_at: "2026-08-20T00:00:00.000Z",
      updated_at: "2026-08-20T00:00:00.000Z",
    }));
  ...
  it("recycleProcessAndReload restores loaded sessions and refuses when busy", async () => {
    let starts = 0;
    const loads = [];
    let rejectPrompt;
    const runtime = new AgentRuntime({
      connect: async () => {
        ...
        return {
          initialize: async () => ({}),
          sessionNew: async () => ({ sessionId: "s-reload" }),
          sessionLoad: async (sessionId, cwd) => {
            loads.push({ sessionId, cwd });
            return { sessionId };
          },
          sessionPrompt: async () => new Promise((_, reject) => {
            rejectPrompt = reject;
          }),
          sessionCancel: () => {
    ...
    const cwd = mkdtempSync(join(tmpdir(), "grok-reload-cwd-"));
    const sessionId = await runtime.createSession(cwd);
    await runtime.recycleProcessAndReload();
    assert.equal(starts, 2);
    assert.equal(loads.length, 1);
    assert.equal(loads[0].sessionId, sessionId);
    assert.equal(runtime.hasSession(sessionId), true);

    const prompt = runtime.send(sessionId, { type: "prompt", message: "hold" });
    await new Promise((resolve) => setTimeout(resolve, 20));
    await assert.rejects(runtime.recycleProcessAndReload(), (error) => (
      error instanceof AgentCommandError && error.status === 409 && error.code === "runtime_busy"
    ));
    await runtime.send(sessionId, { type: "abort" }).catch(() => undefined);
