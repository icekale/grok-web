...
describe("AgentRuntime", () => {
  ...
  it("sends every thinking level including off and preserves state on failure", async () => {
    const modes = [];
    let rejectOff = false;
    const runtime = new AgentRuntime({
      connect: async () => ({
        ...
        sessionSetMode: async (_sessionId, modeId) => {
          modes.push(modeId);
          if (rejectOff && modeId === "off") throw new Error("mode unsupported");
        },
        onSessionUpdate: () => () => {},
        onPermission: () => () => {},
        completePermission() {},
    ...
    rejectOff = true;
    await assert.rejects(runtime.send(sessionId, { type: "set_thinking_level", level: "off" }), /mode unsupported/);
    assert.equal((await runtime.send(sessionId, { type: "get_state" })).thinkingLevel, "off");
  });
  ...
  it("passes a sanitized environment at the default spawn boundary", async () => {
    const source = await readFile(new URL("./runtime.ts", import.meta.url), "utf8");
    assert.match(source, /env:\s*grokAgentEnv\(\)/);
  });
