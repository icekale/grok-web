...
describe("AgentRuntime", () => {
  ...
  it("lists the default Grok tool preset as active", async () => {
    ...
    assert.ok(tools.some((tool) => tool.name === "grep" && tool.active === false));
  });
  ...
  it("maps set_tools through session/set_config_option and returns the updated state", async () => {
    ...
    assert.ok(tools.some((tool) => tool.name === "read" && tool.active === true));
    assert.ok(tools.some((tool) => tool.name === "bash" && tool.active === false));
    const listed = await runtime.send(sessionId, { type: "get_tools" });
    assert.ok(listed.some((tool) => tool.name === "grep" && tool.active === true));
    assert.ok(listed.some((tool) => tool.name === "bash" && tool.active === false));
  });
