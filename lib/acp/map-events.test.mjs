...
describe("AcpTurnMapper", () => {
  ...
  it("attaches Grok turn usage onto message_end", () => {
    const mapper = new AcpTurnMapper();
    mapper.push({ sessionUpdate: "agent_message_chunk", content: { type: "text", text: "hi" } });
    mapper.push({
    ...
    const ended = mapper.endTurn().find((event) => event.type === "message_end");
    assert.deepEqual(ended?.message?.usage, {
      input: 200,
      output: 40,
      cacheRead: 800,
      cacheWrite: 0,
  ...
  it("skips bash description from mapper-emitted partialResult without hand-built rawInput", () => {
    const mapper = new AcpTurnMapper();
    const events = [];
    for (const line of readFileSync(join(fixtures, "tool-bash.jsonl"), "utf8").split(/\r?\n/)) {
      if (!line.trim()) continue;
    ...
    const execUpdates = events.filter((event) => event.type === "tool_execution_update");
    assert.ok(execUpdates.length >= 2);
    const descriptionUpdate = execUpdates.find((event) => (
      toolResultText(event.partialResult) === "List files"
    ));
    assert.ok(descriptionUpdate);
    assert.ok(
      descriptionUpdate.partialResult.rawInput || descriptionUpdate.partialResult.input,
      "partialResult must keep ACP rawInput so applyToolOutputUpdate can skip description",
    );
    let output = { text: "" };
    for (const event of execUpdates) {
      output = applyToolOutputUpdate(output, event.partialResult, event.toolName);
    }
    assert.equal(output.text, "a.ts\n");
  ...
  it("maps auto-compact and failed retry updates to composer events", () => {
    const mapper = new AcpTurnMapper();
    const started = mapper.push({
      sessionUpdate: "auto_compact_started",
      tokens_used: 436010,
      context_window: 500000,
      percentage: 87,
      reason: "Context window 87% full",
    });
    assert.deepEqual(started.map((event) => event.type), ["auto_compaction_start", "context_usage"]);
    assert.deepEqual(started[1].contextUsage, {
      tokens: 436010,
    ...
    assert.equal(failed[0].type, "auto_compaction_end");
    assert.match(failed[0].errorMessage, /new session/);
    const retry = mapper.push({
    ...
    assert.deepEqual(retry, [{
      type: "prompt_error",
      errorMessage: "API error (status 400 Bad Request): Request failed (HTTP 400).",
    }]);
    const completed = mapper.push({
      sessionUpdate: "turn_completed",
      stop_reason: "error",
