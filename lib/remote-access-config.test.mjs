...
test("envPasswordEnabled requires 12 to 256 characters", async () => {
  const { envPasswordEnabled } = await load();
  assert.equal(envPasswordEnabled(undefined), false);
  assert.equal(envPasswordEnabled(""), false);
  assert.equal(envPasswordEnabled("short"), false);
  assert.equal(envPasswordEnabled("x".repeat(11)), false);
  assert.equal(envPasswordEnabled("x".repeat(12)), true);
  assert.equal(envPasswordEnabled("env-password-ok"), true);
  assert.equal(envPasswordEnabled("x".repeat(256)), true);
  assert.equal(envPasswordEnabled("x".repeat(257)), false);
});
...
test("listLanIPv4s accepts Node numeric family 4", () => {
  const source = readFileSync(new URL("./remote-access-config.ts", import.meta.url), "utf8");
  assert.match(source, /family === "IPv4" \|\| family === 4/);
});
