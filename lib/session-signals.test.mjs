...
describe("parseChatHistoryActivity", () => {
  ...
  it("does not treat an unchunked UUID tail as a chunk suffix", () => {
    const unchunked = "call-aaaaaaaa-bbbb-cccc-dddd-123456789012";
    const otherChunked = "call-bbbbbbbb-cccc-dddd-eeee-ffffffffffff-3";
    const raw = [
      JSON.stringify({ type: "tool_result", tool_call_id: unchunked, content: "self" }),
      JSON.stringify({ type: "tool_result", tool_call_id: `${unchunked}-3`, content: "chunk" }),
      JSON.stringify({ type: "tool_result", tool_call_id: otherChunked, content: "other" }),
    ].join("\n");
    assert.deepEqual(parseChatHistoryActivity(raw), { userMessages: 0, toolCalls: 2 });
  });
