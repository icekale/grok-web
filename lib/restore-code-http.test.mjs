...
test("malformed JSON is rejected before preflight RPCs", async () => {
  let listed = false;
  const injected = deps(mkdtempSync(join(tmpdir(), "restore-code-")));
  injected.listWorktrees = async () => { listed = true; return { worktrees: [] }; };
  const response = await createRestoreCodeHandlers(injected).POST(new Request("http://127.0.0.1", { method: "POST", body: "{" }), { id: "session-1234" });
