...
import { hasGrokApiKey, readGrokAuth } from "./grok-settings/home-config.ts";
...
describe("auth HTTP routes", () => {
  ...
  it("GET /api/auth/providers ignores model-table api_key values", async () => {
    const home = mkdtempSync(join(tmpdir(), "grok-login-model-key-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    writeFileSync(join(home, "config.toml"), `[model."cpa/grok-4.6"]\napi_key = "model-secret"\n`);
    setAgentRuntime({
      authCheck: async () => ({ authenticated: false }),
    });
    try {
      const { GET } = await jiti.import("./auth-providers-http.ts");
      const res = await GET();
      const body = await res.json();
      assert.equal(res.status, 200);
      assert.equal(body.providers[0].loggedIn, false);
      const { GET: getKeys } = await jiti.import("./auth-all-providers-http.ts");
  ...
  it("GET /api/auth/all-providers contains only xai.api_key", async () => {
    const runtime = createRuntime();
    setAgentRuntime(runtime);
    const { GET } = await jiti.import("./auth-all-providers-http.ts");
    const res = await GET();
    const body = await res.json();
    assert.equal(res.status, 200);
  ...
  it("POST /api/auth/logout calls through", async () => {
    ...
    try {
      ...
      setAgentRuntime(runtime);
      await runtime.authenticate("xai.api_key");
      const { POST } = await jiti.import("./auth-logout-http.ts");
  ...
  it("POST /api/auth/logout preserves custom model and MCP keys", async () => {
    const home = mkdtempSync(join(tmpdir(), "grok-logout-model-key-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    writeFileSync(join(home, "config.toml"), `api_key = "official-secret"
[model."cpa/grok-4.6"]
api_key = "model-secret"
[mcp_servers.docs]
api_key = "mcp-secret"
`);
    try {
      ...
      assert.doesNotMatch(text, /official-secret/);
      assert.match(text, /api_key = "model-secret"/);
      assert.match(text, /api_key = "mcp-secret"/);
    } finally {
      if (prev === undefined) delete process.env.GROK_HOME;
      else process.env.GROK_HOME = prev;
    }
  ...
  it("DELETE /api/auth/xai.api_key preserves custom Provider keys", async () => {
    const home = mkdtempSync(join(tmpdir(), "grok-delete-api-key-"));
    const prev = process.env.GROK_HOME;
    process.env.GROK_HOME = home;
    writeFileSync(join(home, "config.toml"), `api_key = "official-secret"
[model."cpa/grok-4.6"]
...
`);
    try {
      setAgentRuntime({ authLogout: async () => undefined });
      const { DELETE } = await jiti.import("./auth-api-key-http.ts");
      const res = await DELETE(new Request("http://127.0.0.1/api/auth/xai.api_key", { method: "DELETE" }), {
  ...
  it("GET /api/auth/login for unknown provider emits SSE error", async () => {
    const runtime = createRuntime();
    setAgentRuntime(runtime);
    const { GET } = await jiti.import("./auth-login-http.ts");
  ...
  it("GET /api/auth/login for grok.com emits auth and device_code then succeeds after submit_code", async () => {
    ...
    const runtime = spy.runtime;
    setAgentRuntime(runtime);
    const { GET, POST } = await jiti.import("./auth-login-http.ts");
    const abort = new AbortController();
    const res = await GET(new Request("http://127.0.0.1/api/auth/login/grok.com", { signal: abort.signal }), {
      params: Promise.resolve({ provider: "grok.com" }),
    });
  ...
  it("POST/GET/DELETE /api/auth/api-key/xai.api_key persist without exposing the key", async () => {
    ...
    try {
      const runtime = createRuntime();
      setAgentRuntime(runtime);
      const { GET, POST, DELETE } = await jiti.import("./auth-api-key-http.ts");
  ...
  it("DELETE /api/auth/api-key leaves a model-table api_key untouched", async () => {
    ...
    try {
      ...
      assert.equal(deleted.status, 200);
      assert.equal(hasGrokApiKey(home), false);
      assert.match(readFileSync(join(home, "config.toml"), "utf8"), /api_key = "model-secret"/);
    } finally {
      if (prev === undefined) delete process.env.GROK_HOME;
      else process.env.GROK_HOME = prev;
    }
