...
test("ignores a delta with no event type", () => {
  const state = streamReducer(INITIAL_STREAMING_STATE, { type: "start" });
  assert.equal(streamReducer(state, { type: "delta", event: undefined }).isStreaming, true);
});
...
test("a later start begins a new assistant instead of replaying the previous turn", () => {
  let state = streamReducer(INITIAL_STREAMING_STATE, { type: "start" });
  state = delta(state, { type: "text_start", contentIndex: 0 });
  state = delta(state, { type: "text_delta", contentIndex: 0, delta: "Hel" });
  const again = streamReducer(state, { type: "start" });
  assert.deepEqual(again.streamingMessage.content, []);
  const continued = delta(again, { type: "text_start", contentIndex: 0 });
  const typed = delta(continued, { type: "text_delta", contentIndex: 0, delta: "lo" });
  assert.deepEqual(typed.streamingMessage.content, [{ type: "text", text: "lo" }]);
});

test("ACP live updates stream after start without a reconnect snapshot", () => {
  let state = streamReducer(INITIAL_STREAMING_STATE, { type: "start" });
  state = delta(state, { type: "thinking_start", contentIndex: 0 });
  state = delta(state, { type: "thinking_delta", contentIndex: 0, delta: "Plan" });
  state = delta(state, { type: "text_start", contentIndex: 1 });
