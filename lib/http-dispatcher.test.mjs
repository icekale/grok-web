...
test("configures HTTP_PROXY, HTTPS_PROXY, and NO_PROXY for global fetch", async (t) => {
  ...
  const proxy = createServer((req, res) => {
    // undici >= 8.7 sends http:// targets to the proxy in absolute-form
    // (no CONNECT tunnel); https:// targets still use a CONNECT tunnel.
  ...
  assert.deepEqual(proxyRequests, ["http://target.invalid/through-http-proxy"]);
  ...
  const proxiedRequestCount = proxyRequests.length;
  await assert.rejects(fetch("http://bypass.invalid:9/no-proxy", {
    signal: AbortSignal.timeout(2_000),
  }));
  assert.equal(proxyRequests.length, proxiedRequestCount);
});
