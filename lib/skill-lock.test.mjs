...
test("annotates only lock entries that exist in the matching Pi scope", () => {
  ...
  try {
    ...
    const agentsSkillPath = join(cwd, ".agents", "skills", "find-skills", "SKILL.md");
    const manualSkillPath = join(agentDir, "skills", "manual", "SKILL.md");
    const otherAgentSkillPath = join(root, "other-agent", "tts", "SKILL.md");

    for (const path of [globalSkillPath, projectSkillPath, agentsSkillPath, manualSkillPath, otherAgentSkillPath]) {
    ...
    const annotated = annotateSkillsWithInstallInfo(
      [
        ...
        makeSkill("find-skills", agentsSkillPath, "project"),
    ...
    assert.deepEqual(annotated[2].install, annotated[1].install);
    assert.equal(annotated[3].install, undefined);
    assert.equal(annotated[4].install, undefined);
  } finally {
