...
test("event handler preserves headers and abort cleanup", async (t) => {
  ...
  const originalSubscribe = AgentRuntime.prototype.subscribeSequenced;
  AgentRuntime.prototype.subscribeSequenced = function subscribeSequenced(sessionId, listener) {
  ...
  t.after(() => {
    AgentRuntime.prototype.subscribeSequenced = originalSubscribe;
    resetAgentRuntime();
  });
