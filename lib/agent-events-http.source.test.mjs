...
const agentEventsSource = await readFile(new URL("./agent-events-http.ts", import.meta.url), "utf8");
const runningEventsSource = await readFile(new URL("./agent-running-events-http.ts", import.meta.url), "utf8");
const agentEventStreamSource = await readFile(new URL("./agent-event-stream.ts", import.meta.url), "utf8");
...
test("agent SSE wraps the ACP runtime and disables response buffering", () => {
  ...
  assert.match(agentEventsSource, /runtime\.subscribeSequenced\(id, listener/);
  assert.match(agentEventsSource, /runtime\.getSessionSnapshot\(id\)/);
  assert.doesNotMatch(agentEventsSource, /streamingMessage: null/);
