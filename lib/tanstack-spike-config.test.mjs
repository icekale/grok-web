...
const startupGuardPlugin = await readFile(new URL("../src/plugins/server-startup-guard.ts", import.meta.url), "utf8");

test("registers Nitro lifecycle plugins before the server listens", () => {
  assert.match(viteConfig, /"src\/plugins\/acp-runtime-cleanup\.ts"/);
  assert.match(viteConfig, /"src\/plugins\/server-startup-guard\.ts"/);
  assert.ok(viteConfig.indexOf("acp-runtime-cleanup") < viteConfig.indexOf("server-startup-guard"));
  ...
  assert.match(startupGuardPlugin, /assertServerBindAllowed/);
  assert.match(startupGuardPlugin, /isWebPasswordEnabled/);
});

test("registers one Nitro ACP cleanup plugin", () => {
  assert.match(viteConfig, /acp-runtime-cleanup/);
  assert.match(cleanupPlugin, /definePlugin/);
  assert.match(cleanupPlugin, /hooks\.hook\(["']close["']/);
  assert.match(cleanupPlugin, /disposeAgentRuntime/);
});
