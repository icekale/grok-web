...
import { clearDefaultReasoningEffort, configHasModelSection, grokSettingsPickerId, pinDefaultReasoningEffort, renderGrokModelTable, syncSettingsModelsToGrokConfig } from "./grok-model-table.ts";
...
test("clearDefaultReasoningEffort removes the process-wide pin and leaves other tables", () => {
  const text = [
    `[models]`,
    `default_reasoning_effort = "xhigh"`,
    `default = "cpa/grok-4.6"`,
    ``,
    `[model."cpa/grok-4.6"]`,
    `model = "grok-4.6"`,
    ``,
  ].join("\n");
  const next = clearDefaultReasoningEffort(text);
  assert.doesNotMatch(next, /default_reasoning_effort/);
  assert.match(next, /\[models\][\s\S]*default = "cpa\/grok-4\.6"/);
  assert.match(next, /\[model\."cpa\/grok-4\.6"\][\s\S]*model = "grok-4\.6"/);
  assert.equal(clearDefaultReasoningEffort(next), next);
});

test("pinDefaultReasoningEffort writes the spawn fallback instead of leaving CPA on low", () => {
  const text = [
    `[models]`,
    `default = "cpa/grok-4.6"`,
    ``,
  ...
  const pinned = pinDefaultReasoningEffort(text, "xhigh");
  assert.match(pinned, /\[models\]\s*\ndefault_reasoning_effort = "xhigh"/);
  assert.match(pinned, /default = "cpa\/grok-4\.6"/);
  assert.equal(pinDefaultReasoningEffort(pinned, "high"), pinned.replace("xhigh", "high"));
  assert.doesNotMatch(pinDefaultReasoningEffort(pinned, undefined), /default_reasoning_effort/);
});

test("custom Grok 4.5 tables advertise the official trio with high as default", () => {
  const table = renderGrokModelTable({ providerId: "cpa", id: "grok-4.5", name: "Grok 4.5" });
  ...
  assert.doesNotMatch(table, /xhigh/);
});

test("imagine and composer tables do not advertise effort", () => {
  const imagine = renderGrokModelTable({ providerId: "cpa", id: "grok-imagine-image-2.0", name: "Grok Imagine Image 2.0" });
  const composer = renderGrokModelTable({ providerId: "cpa", id: "grok-composer-2.5-fast", name: "Grok Composer 2.5 Fast" });
  assert.doesNotMatch(imagine, /supports_reasoning_effort/);
  assert.doesNotMatch(composer, /reasoning_efforts/);
});
...
test("custom Grok 4.6 tables preserve exactly its four reasoning efforts", () => {
  ...
  assert.deepEqual(
    [...table.matchAll(/\{ id = "([^"]+)", value = "([^"]+)", label = "([^"]+)", default = (true|false) \}/g)]
      .map((match) => match.slice(1)),
    [
      ["xhigh", "xhigh", "Extra High Effort", "true"],
      ["high", "high", "High Effort", "false"],
      ["medium", "medium", "Medium Effort", "false"],
      ["low", "low", "Low Effort", "false"],
    ],
  );
...
test("sync writes namespaced cpa tables and leaves an existing grok-4.6 override", () => {
  ...
  assert.match(text, /\[model\."cpa\/grok-4\.5"\][\s\S]*supports_reasoning_effort = true/);
