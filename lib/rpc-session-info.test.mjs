...
const { getRpcSessionInfos, getRunningRpcSessionIds } = await jiti.import("./rpc-manager.ts");
...
test("running snapshot includes cached live subagent session ids", (t) => {
  const previousRegistry = globalThis.__piSessions;
  const parent = {
    ...makeRuntimeSession({
      id: "root",
      filePath: join(tmpdir(), "root.jsonl"),
      running: false,
      entries: [],
    }),
    getLiveSubagentSessionIds: () => ["child-a", "child-b"],
  };
  globalThis.__piSessions = new Map([["root", parent]]);
  t.after(() => {
    globalThis.__piSessions = previousRegistry;
  });

  assert.deepEqual([...getRunningRpcSessionIds()].sort(), ["child-a", "child-b"]);
});
