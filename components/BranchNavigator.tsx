  ...
  return (
    <div>
      ...
      >
        ...
        {/* Skipped indicator */}
        {skipped > 0 && (
          <span style={{ fontSize: "var(--text-meta)", color: "var(--text-dim)", marginRight: 5, flexShrink: 0 }}>
            +{skipped}
          </span>
        )}
...
export function BranchNavigator({ tree, activeLeafId, onLeafChange, inline, containerRef, open: openProp, onToggle, hasSession, compact, hideInlineButton }: Props) {
  ...
  const branchIcon = (
    <GitBranch size={12} style={{ color: hasContent ? "var(--accent)" : "var(--text-dim)", flexShrink: 0 }} aria-hidden="true" />
  );

  const chevron = (
    <ChevronDown size={10} color="var(--text-dim)" strokeWidth={1.6} style={{ marginLeft: 2, transform: open ? "rotate(180deg)" : "none", transition: "transform 0.15s" }} aria-hidden="true" />
  );
