...
test("sidebar recomposition preserves worktree switching", () => {
  ...
  assert.doesNotMatch(sidebar, /codex-project-daily-links/);
  assert.doesNotMatch(sidebar, /onOpenSettings/);
});

test("session overflow menu keeps rename, archive, and delete", () => {
  assert.match(sidebar, /\{t\("sidebar\.rename"\)\}<\/button>[\s\S]*?\{t\("sidebar\.archiveSession"\)\}<\/button>[\s\S]*?\{t\("sidebar\.delete"\)\}<\/button>/);
  assert.doesNotMatch(sidebar, /sidebar\.renameCommand/);
  assert.doesNotMatch(sidebar, /sidebar\.exportSession/);
  assert.doesNotMatch(sidebar, /codex-menu-command/);
});
